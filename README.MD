...
An automated tool for interacting with the Plume Testnet, designed to bridge assets from supported networks (Base, Arbitrum, Optimism) to Plume and perform swaps to achieve a target transaction count.
...
### Software Workflow
1. Loads wallets and proxies from files.
2. Initializes clients with random user agents and proxies.
3. Checks Plume balance; skips bridging if balance exists.
4. Selects a random source network (Base, Arbitrum, or Optimism) with a minimum balance of 0.0008 ETH for bridging.
5. Bridges 95% of the available balance to Plume.
6. Performs swaps on Plume until 400 transactions are reached.
7. Handles proxy errors with automatic replacement (if enabled).
8. Logs results and errors to the console and database.
...
Автоматизированный инструмент для взаимодействия с тестовой сетью Plume, предназначенный для переброски активов из поддерживаемых сетей (Base, Arbitrum, Optimism) в Plume и выполнения свопов для достижения целевого количества транзакций.
...
- Выполнение свапов в Plume для достижения 400 транзакций
...
1. Загружает кошельки и прокси из файлов.
2. Инициализирует клиенты с случайными пользовательскими агентами и прокси.
3. Проверяет баланс в Plume; пропускает переброску, если баланс есть.
4. Выбирает случайную исходную сеть (Base, Arbitrum или Optimism) с минимальным балансом 0.0008 ETH для переброски.
5. Перебрасывает 95% доступного баланса в Plume.
6. Выполняет свопы в Plume до достижения 400 транзакций.
7. Обрабатывает ошибки прокси с автоматической заменой (если включено).
8. Логирует результаты и ошибки в консоль и базу данных.
...
- **Свопы**: Выполняет свопы в Plume, пока кошелек не достигнет 400 транзакций.
